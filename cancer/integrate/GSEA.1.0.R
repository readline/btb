...
# p-vals plot
      ...
      if (non.interactive.run == F) {  
           if (.Platform$OS.type == "windows") {
               ...
               savePlot(filename = glob.filename, type ="svg", device = dev.cur())
           } else if (.Platform$OS.type == "unix") {
               dev.off()
           }
      } else {
...
if (output.directory != "")  {
      ...
      if (non.interactive.run == F) {  
           if (.Platform$OS.type == "windows") {
               ...
               savePlot(filename = gs.filename, type ="svg", device = dev.cur())
           } else if (.Platform$OS.type == "unix") {
               dev.off()
           }
      } else {
...
#   max.sets.phen2 <- length(set.table.phen2[,1])
   ...
   if (non.interactive.run == F) {  
        if (.Platform$OS.type == "windows") {
            ...
            savePlot(filename = filename, type ="svg", device = dev.cur())
        } else if (.Platform$OS.type == "unix") {
            dev.off()
        }
   } else {
