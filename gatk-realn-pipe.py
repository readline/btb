#!/usr/bin/env python
# =============================================================================
# Filename: gatk-realn-pipe.py
# Version: 
# Author: Kai Yu - finno@live.cn
# https://github.com/readline
# Last modified: 2015-07-04 17:07
# Description: 
# 
# =============================================================================
...
def main():
    ...
    cmd4 = 'java -Xmx8g -Djava.io.tmpdir=%s -jar %s \
               ...
               -known %s\
               -I %s.rg.rmdup.bam \
               -o %s.rg.rmdup.intervals' %(tmpdir, gatk, fasta, region, known1, known2, prefix, prefix)
    if os.path.exists('%s.rg.rmdup.intervals.ok'%prefix) == False:
        print
        print cmdExec(cmd4)
        tmp = open('%s.rg.rmdup.intervals.ok'%prefix, 'w')
        tmp.close()
    else:
        print
    ...
    cmd5 = 'java -Xmx8g -Djava.io.tmpdir=%s -jar %s \
               ...
               -known %s \
               -known %s \
               -targetIntervals %s.rg.rmdup.intervals \
               -I %s.rg.rmdup.bam \
               -o %s.rg.rmdup.realn.bam' %(tmpdir, gatk, fasta, region, known1, known2, prefix, prefix, prefix)
    if os.path.exists('%s.rg.rmdup.realn.bam.ok'%prefix) == False:
        print
        print cmdExec(cmd5)
        tmp = open('%s.rg.rmdup.realn.bam.ok'%prefix, 'w')
        tmp.close()
        os.remove('%s.rg.rmdup.metrics'%prefix)
        os.remove('%s.rg.rmdup.intervals'%prefix)
    ...
    cmd6 = 'java -Xmx8g -Djava.io.tmpdir=%s -jar %s \
               ...
               -knownSites %s \
               -knownSites %s \
               -I %s.rg.rmdup.realn.bam \
               -o %s.rg.rmdup.realn.recal.grp' %(tmpdir, gatk, fasta, region, known1, known2, known3, prefix, prefix)
    if os.path.exists('%s.rg.rmdup.realn.recal.grp.ok'%prefix) == False:
        print
        print cmdExec(cmd6)
        tmp = open('%s.rg.rmdup.realn.recal.grp.ok'%prefix, 'w')
        tmp.close()
    else:
        print
    ...
    cmd7 = 'java -Xmx8g -Djava.io.tmpdir=%s -jar %s \
               ...
               -R %s \
               %s \
               -BQSR %s.rg.rmdup.realn.recal.grp \
               -I %s.rg.rmdup.realn.bam \
    ...
    if os.path.exists('%s.phase1.bam.ok'%prefix) == False:
        ...
        tmp.close()
        os.remove('%s.rg.rmdup.realn.recal.grp'%prefix)
        os.remove('%s.rg.rmdup.realn.bam'%prefix)
        os.remove('%s.rg.rmdup.realn.bai'%prefix)
